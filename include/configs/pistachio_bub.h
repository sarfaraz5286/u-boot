...
#define CONFIG_FDT_MAX_SIZE		0x00010000
#define CONFIG_SYS_SPI_U_BOOT_OFFS 	0x00100000

/*
 * We'll place FDT in the last 64KB of aligned GRAM:
 * 0x9A050000
 */
#define CONFIG_SPL_FDT_START_ADDR	(((CONFIG_SYS_GRAM_BASE) + \
					(CONFIG_SYS_GRAM_SIZE) - \
					(CONFIG_FDT_MAX_SIZE)) \
					& (-0x10000))
/* TODO: temp as Makefile cannot do evaluate CONFIG values */
#define CONFIG_SPL_FDT_OFFSET		0x00050000

/*
 * Place BSS imediatelly before DTB
 * _bss_end has to be the same as CONFIG_SPL_FDT_START_ADDR
 * 0x9A040000
 */
#define CONFIG_SPL_BSS_START_ADDR	((CONFIG_SPL_FDT_START_ADDR) - \
