/*
 * DDR3 initialization code for MIPS32 CPU-core SPL
 *
...
int init_ddr(void)
{
	...
	temp_rw_val = readl(TOPLEVEL_REGS + DDR_CTRL_OFFSET);
	/* Set CLK_EN = 1 */
	temp_rw_val |= 0x2;
	writel(temp_rw_val, TOPLEVEL_REGS + DDR_CTRL_OFFSET);
	readl(TOPLEVEL_REGS + DDR_CTRL_OFFSET);
	/*
	 * Reset the AXI bridge and DDR Controller in case any spurious
	 * writes have already happened to DDR
	 */
	/* Drive the 3 resets low */
	writel(0x00000002, TOPLEVEL_REGS + DDR_CTRL_OFFSET);
	readl(TOPLEVEL_REGS + DDR_CTRL_OFFSET);
	readl(TOPLEVEL_REGS + DDR_CTRL_OFFSET);

	/* And release */
	writel(0x0000000F, TOPLEVEL_REGS + DDR_CTRL_OFFSET);
	...
	writel(0x000000C8, DDR_PCTL + DDR_PCTL_TOGCNT1U_OFFSET);
	/* TINIT: t_init Timing Register: at least 200us 200h C8h */
	writel(0x000000C8, DDR_PCTL + DDR_PCTL_TINIT_OFFSET);
	/* TRSTH: Reset High Time Register DDR3 ONLY */
	writel(0x000001F4, DDR_PCTL + DDR_PCTL_TRSTH_OFFSET);
	/* TOGCNT100N: Toggle Counter 100N Register: 20d, 14h*/
	writel(0x00000014, DDR_PCTL + DDR_PCTL_TOGG_CNTR_100NS_OFFSET);
	 ...
	 */
	writel(0x0000004B, DDR_PCTL + DDR_PCTL_DTUAWDT_OFFSET);
	/* MCFG
	 * 0 BL 1 -> 8 fixed
	 * 1 RDRIMM 0
	 ...
	 */
	writel(0x00060021, DDR_PCTL + DDR_PCTL_MCFG_OFFSET);
	 ...
	 */
	writel(0x00000100, DDR_PCTL + DDR_PCTL_MCFG1_OFFSET);
	/* DCR DRAM Config
	 * 2:0 SDRAM => DDR3 3
	 * 3 DDR 8 Bank 1
	 ...
	 * 31 TPD LPDDR2 0
	 */
	writel(0x0000000B, DDR_PHY + DDRPHY_DCR_OFFSET);
	 ...
	 * 31:16 Reserved
	 */
	writel(0x00001520, DDR_PHY + DDRPHY_MR_OFFSET);
	 ...
	 * 31:16 Reserved
	 */
	writel(0x00000004, DDR_PHY + DDRPHY_EMR_OFFSET);
	 ...
	 * 31:11 Reserved
	 */
	writel(0x00000440, DDR_PHY + DDRPHY_EMR2_OFFSET);
	/* MR3: DDR3 mode register 3
	 * 1:0 MPRLOC 00
	 * 2 MPR 0
	 */
	writel(0x00000000, DDR_PHY + DDRPHY_EMR3_OFFSET);
	 ...
	 */
	writel(0x00000000, DDR_PHY + DDRPHY_DTAR_OFFSET);
	/* DSGCR
	 * 0 PUREN Def 1
	 * 1 BDISEN Def 1
	 * 2 ZUEN Def 1
	 ...
	 */
	writel(0xFA000927, DDR_PHY + DDRPHY_DSGCR_OFFSET);
	/* Sysnopsys advised 500R pullup/pulldown DQS DQSN */
	writel(0x00000C40, DDR_PHY + DDRPHY_DXCCR_OFFSET);
	 ...
	 */
	writel(0x2A8F6688, DDR_PHY + DDRPHY_DTPR0_OFFSET);
	 ...
	 * 31:30 Reserved
	 */
	writel(0x094006A0, DDR_PHY + DDRPHY_DTPR1_OFFSET);
	 ...
	 * 32:29 Reserved
	 */
	writel(0x10029600, DDR_PHY + DDRPHY_DTPR2_OFFSET);
	 ...
	 * 31 loopback mode 0
	 */
	writel(0x01BC2E02, DDR_PHY + DDRPHY_PGCR_OFFSET);
	...
	writel(0x0000001F, DDR_PHY + DDRPHY_PIR_OFFSET);
	/* PGSR : Wait for DRAM Init Done */
	if (wait_for_completion(DDR_PHY + DDRPHY_PGSR_OFFSET, 0x00000007))
			return DDR_TIMEOUT;
	/* PIR : controller DRAM initialization */
	writel(0x00040001, DDR_PHY + DDRPHY_PIR_OFFSET);
	/* PGSR : Wait for DRAM Init Done */
	...
	writel(0x80000001, DDR_PCTL + DDR_PCTL_POWCTL_OFFSET);
	 ...
	 */
	writel(0x00070101, DDR_PCTL + DDR_PCTL_DFILPCFG0_OFFSET);
	 ...
	 */
	writel(0x00000008, DDR_PCTL + DDR_PCTL_DFIODTCFG_OFFSET);
	 ...
	 */
	writel(0x060600000, DDR_PCTL + DDR_PCTL_DFIODTCFG1_OFFSET);
