...
{
	unsigned long deadline = get_timer(0) + 100;
	bool timeout = false;
	int ret;
...
{
	/*
	 * TODO1: This will be NULL as we don't need it
	 * until DT is available
	 *
	 * if (!snand->dev)
	 *	return -ENODEV;
	 */
...
int spi_nand_register(struct spi_nand *snand, struct nand_flash_dev *flash_ids)
{
	...
	/* TODO1: Using CONFIG to specify on flash BBT until DT available */
